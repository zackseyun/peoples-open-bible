"""Original research composition checks against their immutable live baseline."""
import unittest

from tools.textual_restoration.replay_source_predecessor_tests import CHECKPOINT, replay


class SourceCompositionHistoricalTests(unittest.TestCase):
    def test_original_7_checks_in_shared_19_test_archival_replay(self):
        # Both archival wrappers share one cached run, not 19 executions each.
        result = replay()
        self.assertEqual(result["checkpoint"], CHECKPOINT)
        self.assertEqual(result["suite_test_counts"]["source_composition"], 7)
